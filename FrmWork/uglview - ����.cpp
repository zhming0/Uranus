...
public:
    ...
    void run()
    {
        ...
        if(f.open(f.ReadOnly)){
            ...
            int *vertexSize[2];
            vertexSize[0]=&v->vertexLastSize;
            vertexSize[1]=&v->vertexLastSize2;
            (*vertexSize[0])=(*vertexSize[1])=vertexListMax+1;
            float *coord[2],*color[2];
            int pl=0;
            qDebug()<<"read Points"<<n;
            n=0;
            for(int z=0;z<c;z++)
            {
                ...
                for(int i=0;i<len;i++)
                {
                    ...
                    if(t>min)
                    {
                        ...
                        if(col>0.05)
                        {
                            if((*vertexSize[0])>=vertexListMax)
                            {
                                (*vertexSize[0])=0;
                                (*vertexSize[1])=0;
                                v->vertexCoord.append(coord[0]=new float[3*vertexListMax]);
                                v->vertexColor.append(color[0]=new float[4*vertexListMax]);
                                v->vertexCoord.append(coord[1]=new float[3*vertexListMax]);
                                v->vertexColor.append(color[1]=new float[4*vertexListMax]);
                            }
                            pl=(pl+1)%2;        //starts with 1, i.e. second list
                            const static int SHAKE=1000;
                            float rd=(qrand()%SHAKE/1.0f)/SHAKE;
                            coord[pl][(*vertexSize[pl])*3]=_w*(i%w+rd/10)-1;
                            coord[pl][(*vertexSize[pl])*3+1]=_h*(i/w+rd/10)-1;
                            coord[pl][(*vertexSize[pl])*3+2]=inc*(z+rd)-1;
                            color[pl][(*vertexSize[pl])*4]=color[pl][(*vertexSize[pl])*4+1]=color[pl][(*vertexSize[pl])*4+2]=1;
                            col=col*inc*0.7;
                            if(col>1)
                                col=1;
                            color[pl][(*vertexSize[pl])*4+3]=col;
                            (*vertexSize[pl])++;
                        }
                    }
                }
            ...
            f.read(&b,1);
            f.close();
            qDebug()<<"saved points"<<n;
            //qDebug()<<v->vertexLastSize<<v->vertexLastSize2<<v->vertexCoord.count()<<ccc;
        }
        v->status="";
        opening=false;
    }
